...
void animation::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (thumbnail_ == nullptr) { s.store_field("thumbnail", "null"); } else { thumbnail_->store(s, "thumbnail"); }
    if (animation_ == nullptr) { s.store_field("animation", "null"); } else { animation_->store(s, "animation"); }
    s.store_class_end();
  }
}
...
void animations::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    s.store_class_begin(field_name, "animations");
    {
        ...
        const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]";
        s.store_class_begin("animations", vector_name.c_str());
        for (std::uint32_t i = 0; i < multiplicity; i++) {
            if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } }
        s.store_class_end(); }
    s.store_class_end();
  }
}
...
void audio::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("file_name", file_name_);
    s.store_field("mime_type", mime_type_);
    if (album_cover_thumbnail_ == nullptr) { s.store_field("album_cover_thumbnail", "null"); } else { album_cover_thumbnail_->store(s, "album_cover_thumbnail"); }
    if (audio_ == nullptr) { s.store_field("audio", "null"); } else { audio_->store(s, "audio"); }
    s.store_class_end();
  }
}
...
void authenticationCodeInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (next_type_ == nullptr) { s.store_field("next_type", "null"); } else { next_type_->store(s, "next_type"); }
...
void authorizationStateWaitCode::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (code_info_ == nullptr) { s.store_field("code_info", "null"); } else { code_info_->store(s, "code_info"); }
    s.store_class_end();
  }
}
...
void basicGroupFullInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<chatMember>> &v = members_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("members", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void botInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("description", description_);
    { const std::vector<object_ptr<botCommand>> &v = commands_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("commands", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
callStateReady::callStateReady(object_ptr<callProtocol> &&protocol_, std::vector<object_ptr<callConnection>> &&connections_, std::string const &config_, std::string const &encryption_key_, std::vector<std::string> &&emojis_)
...
void callStateReady::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<callConnection>> &v = connections_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("connections", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_field("config", config_);
    s.store_bytes_field("encryption_key", encryption_key_);
    { const std::vector<std::string> &v = emojis_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("emojis", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void chat::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (last_message_ == nullptr) { s.store_field("last_message", "null"); } else { last_message_->store(s, "last_message"); }
    ...
    if (notification_settings_ == nullptr) { s.store_field("notification_settings", "null"); } else { notification_settings_->store(s, "notification_settings"); }
    s.store_field("reply_markup_message_id", reply_markup_message_id_);
    if (draft_message_ == nullptr) { s.store_field("draft_message", "null"); } else { draft_message_->store(s, "draft_message"); }
...
void chatEventMessageEdited::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (old_message_ == nullptr) { s.store_field("old_message", "null"); } else { old_message_->store(s, "old_message"); }
...
void chatEventMemberPromoted::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("user_id", user_id_);
    if (old_status_ == nullptr) { s.store_field("old_status", "null"); } else { old_status_->store(s, "old_status"); }
    if (new_status_ == nullptr) { s.store_field("new_status", "null"); } else { new_status_->store(s, "new_status"); }
    s.store_class_end();
  }
}
...
void chatEventPhotoChanged::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (old_photo_ == nullptr) { s.store_field("old_photo", "null"); } else { old_photo_->store(s, "old_photo"); }
...
void chatEvents::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<chatEvent>> &v = events_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("events", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void chatInviteLinkInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (photo_ == nullptr) { s.store_field("photo", "null"); } else { photo_->store(s, "photo"); }
    s.store_field("member_count", member_count_);
    { const std::vector<std::int32_t> &v = member_user_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("member_user_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
...
void chatMember::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (status_ == nullptr) { s.store_field("status", "null"); } else { status_->store(s, "status"); }
    if (bot_info_ == nullptr) { s.store_field("bot_info", "null"); } else { bot_info_->store(s, "bot_info"); }
    s.store_class_end();
  }
}
...
chatNotificationSettings::chatNotificationSettings(bool use_default_mute_for_, std::int32_t mute_for_, bool use_default_sound_, std::string const &sound_, bool use_default_show_preview_, bool show_preview_)
...
void chatPhoto::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (small_ == nullptr) { s.store_field("small", "null"); } else { small_->store(s, "small"); }
    if (big_ == nullptr) { s.store_field("big", "null"); } else { big_->store(s, "big"); }
    s.store_class_end();
  }
}
...
void chats::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int64_t> &v = chat_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("chat_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void connectedWebsites::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<connectedWebsite>> &v = websites_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("websites", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
deviceTokenApplePushVoIP::deviceTokenApplePushVoIP(std::string const &device_token_, bool is_app_sandbox_)
  : device_token_(std::move(device_token_))
  , is_app_sandbox_(is_app_sandbox_)
{}
...
void draftMessage::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (input_message_text_ == nullptr) { s.store_field("input_message_text", "null"); } else { input_message_text_->store(s, "input_message_text"); }
    s.store_class_end();
  }
}
...
void encryptedPassportElement::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (type_ == nullptr) { s.store_field("type", "null"); } else { type_->store(s, "type"); }
    s.store_bytes_field("data", data_);
    if (front_side_ == nullptr) { s.store_field("front_side", "null"); } else { front_side_->store(s, "front_side"); }
    if (reverse_side_ == nullptr) { s.store_field("reverse_side", "null"); } else { reverse_side_->store(s, "reverse_side"); }
    if (selfie_ == nullptr) { s.store_field("selfie", "null"); } else { selfie_->store(s, "selfie"); }
    { const std::vector<object_ptr<datedFile>> &v = translation_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("translation", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<object_ptr<datedFile>> &v = files_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("files", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void file::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (local_ == nullptr) { s.store_field("local", "null"); } else { local_->store(s, "local"); }
    if (remote_ == nullptr) { s.store_field("remote", "null"); } else { remote_->store(s, "remote"); }
    s.store_class_end();
  }
}
...
void formattedText::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("text", text_);
    { const std::vector<object_ptr<textEntity>> &v = entities_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("entities", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void foundMessages::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<message>> &v = messages_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("messages", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void gameHighScores::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<gameHighScore>> &v = scores_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("scores", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void hashtags::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::string> &v = hashtags_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("hashtags", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void identityDocument::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (expiry_date_ == nullptr) { s.store_field("expiry_date", "null"); } else { expiry_date_->store(s, "expiry_date"); }
    if (front_side_ == nullptr) { s.store_field("front_side", "null"); } else { front_side_->store(s, "front_side"); }
    if (reverse_side_ == nullptr) { s.store_field("reverse_side", "null"); } else { reverse_side_->store(s, "reverse_side"); }
    if (selfie_ == nullptr) { s.store_field("selfie", "null"); } else { selfie_->store(s, "selfie"); }
    { const std::vector<object_ptr<datedFile>> &v = translation_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("translation", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void importedContacts::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int32_t> &v = user_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("user_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    { const std::vector<std::int32_t> &v = importer_count_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("importer_count", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void inlineQueryResultContact::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    if (contact_ == nullptr) { s.store_field("contact", "null"); } else { contact_->store(s, "contact"); }
    if (thumbnail_ == nullptr) { s.store_field("thumbnail", "null"); } else { thumbnail_->store(s, "thumbnail"); }
    s.store_class_end();
  }
}
...
void inlineQueryResultVenue::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    if (venue_ == nullptr) { s.store_field("venue", "null"); } else { venue_->store(s, "venue"); }
    if (thumbnail_ == nullptr) { s.store_field("thumbnail", "null"); } else { thumbnail_->store(s, "thumbnail"); }
    s.store_class_end();
  }
}
...
void inlineQueryResultVideo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    if (video_ == nullptr) { s.store_field("video", "null"); } else { video_->store(s, "video"); }
    s.store_field("title", title_);
    s.store_field("description", description_);
    s.store_class_end();
  }
...
void inlineQueryResultVoiceNote::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    if (voice_note_ == nullptr) { s.store_field("voice_note", "null"); } else { voice_note_->store(s, "voice_note"); }
    s.store_field("title", title_);
    s.store_class_end();
  }
}
...
void inlineQueryResults::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<InlineQueryResult>> &v = results_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("results", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void inputIdentityDocument::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (reverse_side_ == nullptr) { s.store_field("reverse_side", "null"); } else { reverse_side_->store(s, "reverse_side"); }
    if (selfie_ == nullptr) { s.store_field("selfie", "null"); } else { selfie_->store(s, "selfie"); }
    { const std::vector<object_ptr<InputFile>> &v = translation_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("translation", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void inputInlineQueryResultAnimatedGif::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (reply_markup_ == nullptr) { s.store_field("reply_markup", "null"); } else { reply_markup_->store(s, "reply_markup"); }
    if (input_message_content_ == nullptr) { s.store_field("input_message_content", "null"); } else { input_message_content_->store(s, "input_message_content"); }
    s.store_class_end();
  }
}
...
void inputMessagePhoto::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (photo_ == nullptr) { s.store_field("photo", "null"); } else { photo_->store(s, "photo"); }
    if (thumbnail_ == nullptr) { s.store_field("thumbnail", "null"); } else { thumbnail_->store(s, "thumbnail"); }
    { const std::vector<std::int32_t> &v = added_sticker_file_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("added_sticker_file_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_field("width", width_);
    s.store_field("height", height_);
    if (caption_ == nullptr) { s.store_field("caption", "null"); } else { caption_->store(s, "caption"); }
...
void inputMessageVideoNote::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (video_note_ == nullptr) { s.store_field("video_note", "null"); } else { video_note_->store(s, "video_note"); }
    if (thumbnail_ == nullptr) { s.store_field("thumbnail", "null"); } else { thumbnail_->store(s, "thumbnail"); }
    s.store_field("duration", duration_);
    s.store_field("length", length_);
    s.store_class_end();
...
void inputPassportElementPersonalDetails::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (personal_details_ == nullptr) { s.store_field("personal_details", "null"); } else { personal_details_->store(s, "personal_details"); }
    s.store_class_end();
  }
}
...
void inputPassportElementPassport::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (passport_ == nullptr) { s.store_field("passport", "null"); } else { passport_->store(s, "passport"); }
    s.store_class_end();
  }
}
...
void inputPassportElementDriverLicense::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (driver_license_ == nullptr) { s.store_field("driver_license", "null"); } else { driver_license_->store(s, "driver_license"); }
    s.store_class_end();
  }
}
...
void inputPassportElementIdentityCard::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (identity_card_ == nullptr) { s.store_field("identity_card", "null"); } else { identity_card_->store(s, "identity_card"); }
    s.store_class_end();
  }
}
...
void inputPassportElementInternalPassport::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (internal_passport_ == nullptr) { s.store_field("internal_passport", "null"); } else { internal_passport_->store(s, "internal_passport"); }
    s.store_class_end();
  }
}
...
void inputPassportElementUtilityBill::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (utility_bill_ == nullptr) { s.store_field("utility_bill", "null"); } else { utility_bill_->store(s, "utility_bill"); }
    s.store_class_end();
  }
}
...
void inputPassportElementBankStatement::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (bank_statement_ == nullptr) { s.store_field("bank_statement", "null"); } else { bank_statement_->store(s, "bank_statement"); }
    s.store_class_end();
  }
}
...
void inputPassportElementRentalAgreement::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (rental_agreement_ == nullptr) { s.store_field("rental_agreement", "null"); } else { rental_agreement_->store(s, "rental_agreement"); }
    s.store_class_end();
  }
}
...
void inputPassportElementPassportRegistration::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (passport_registration_ == nullptr) { s.store_field("passport_registration", "null"); } else { passport_registration_->store(s, "passport_registration"); }
    s.store_class_end();
  }
}
...
void inputPassportElementTemporaryRegistration::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (temporary_registration_ == nullptr) { s.store_field("temporary_registration", "null"); } else { temporary_registration_->store(s, "temporary_registration"); }
    s.store_class_end();
  }
}
...
void inputPassportElementErrorSourceTranslationFiles::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::string> &v = file_hashes_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("file_hashes", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_bytes_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void inputPersonalDocument::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<InputFile>> &v = files_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("files", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<object_ptr<InputFile>> &v = translation_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("translation", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void inputSticker::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (png_sticker_ == nullptr) { s.store_field("png_sticker", "null"); } else { png_sticker_->store(s, "png_sticker"); }
    s.store_field("emojis", emojis_);
    if (mask_position_ == nullptr) { s.store_field("mask_position", "null"); } else { mask_position_->store(s, "mask_position"); }
    s.store_class_end();
  }
}
...
void invoice::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<labeledPricePart>> &v = price_parts_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("price_parts", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
languagePackInfo::languagePackInfo(std::string const &id_, std::string const &name_, std::string const &native_name_, std::int32_t local_string_count_)
  : id_(std::move(id_))
...
void languagePackStrings::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<languagePackString>> &v = strings_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("strings", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
localFile::localFile(std::string const &path_, bool can_be_downloaded_, bool can_be_deleted_, bool is_downloading_active_, bool is_downloading_completed_, std::int32_t downloaded_prefix_size_, std::int32_t downloaded_size_)
  : path_(std::move(path_))
...
void localizationTargetInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<languagePackInfo>> &v = language_packs_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("language_packs", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void maskPosition::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (point_ == nullptr) { s.store_field("point", "null"); } else { point_->store(s, "point"); }
...
void message::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("chat_id", chat_id_);
    if (sending_state_ == nullptr) { s.store_field("sending_state", "null"); } else { sending_state_->store(s, "sending_state"); }
    s.store_field("is_outgoing", is_outgoing_);
    s.store_field("can_be_edited", can_be_edited_);
    ...
    if (forward_info_ == nullptr) { s.store_field("forward_info", "null"); } else { forward_info_->store(s, "forward_info"); }
    s.store_field("reply_to_message_id", reply_to_message_id_);
    s.store_field("ttl", ttl_);
    ...
    if (content_ == nullptr) { s.store_field("content", "null"); } else { content_->store(s, "content"); }
    if (reply_markup_ == nullptr) { s.store_field("reply_markup", "null"); } else { reply_markup_->store(s, "reply_markup"); }
    s.store_class_end();
  }
}
...
void messageText::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (text_ == nullptr) { s.store_field("text", "null"); } else { text_->store(s, "text"); }
    if (web_page_ == nullptr) { s.store_field("web_page", "null"); } else { web_page_->store(s, "web_page"); }
    s.store_class_end();
  }
}
...
void messageCall::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (discard_reason_ == nullptr) { s.store_field("discard_reason", "null"); } else { discard_reason_->store(s, "discard_reason"); }
    s.store_field("duration", duration_);
    s.store_class_end();
  }
}
...
void messagePaymentSuccessfulBot::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (order_info_ == nullptr) { s.store_field("order_info", "null"); } else { order_info_->store(s, "order_info"); }
...
void messagePassportDataSent::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<PassportElementType>> &v = types_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("types", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void messagePassportDataReceived::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<encryptedPassportElement>> &v = elements_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("elements", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
messageForwardedFromUser::messageForwardedFromUser(std::int32_t sender_user_id_, std::int32_t date_, std::int64_t forwarded_from_chat_id_, std::int64_t forwarded_from_message_id_)
  : sender_user_id_(sender_user_id_)
  , date_(date_)
...
messageForwardedPost::messageForwardedPost(std::int64_t chat_id_, std::string const &author_signature_, std::int32_t date_, std::int64_t message_id_, std::int64_t forwarded_from_chat_id_, std::int64_t forwarded_from_message_id_)
  : chat_id_(chat_id_)
  , author_signature_(std::move(author_signature_))
  , date_(date_)
  , message_id_(message_id_)
...
void networkStatistics::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<NetworkStatisticsEntry>> &v = entries_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("entries", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void networkStatisticsEntryFile::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (network_type_ == nullptr) { s.store_field("network_type", "null"); } else { network_type_->store(s, "network_type"); }
...
void pageBlockTitle::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (title_ == nullptr) { s.store_field("title", "null"); } else { title_->store(s, "title"); }
    s.store_class_end();
  }
}
...
void pageBlockSubtitle::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (subtitle_ == nullptr) { s.store_field("subtitle", "null"); } else { subtitle_->store(s, "subtitle"); }
    s.store_class_end();
  }
}
...
void pageBlockHeader::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (header_ == nullptr) { s.store_field("header", "null"); } else { header_->store(s, "header"); }
    s.store_class_end();
  }
}
...
void pageBlockSubheader::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (subheader_ == nullptr) { s.store_field("subheader", "null"); } else { subheader_->store(s, "subheader"); }
    s.store_class_end();
  }
}
...
void pageBlockFooter::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (footer_ == nullptr) { s.store_field("footer", "null"); } else { footer_->store(s, "footer"); }
    s.store_class_end();
  }
}
...
void pageBlockList::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<RichText>> &v = items_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("items", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
pageBlockAnimation::pageBlockAnimation(object_ptr<animation> &&animation_, object_ptr<RichText> &&caption_, bool need_autoplay_)
  : animation_(std::move(animation_))
  , caption_(std::move(caption_))
...
pageBlockVideo::pageBlockVideo(object_ptr<video> &&video_, object_ptr<RichText> &&caption_, bool need_autoplay_, bool is_looped_)
  : video_(std::move(video_))
  , caption_(std::move(caption_))
  , need_autoplay_(need_autoplay_)
...
pageBlockEmbedded::pageBlockEmbedded(std::string const &url_, std::string const &html_, object_ptr<photo> &&poster_photo_, std::int32_t width_, std::int32_t height_, object_ptr<RichText> &&caption_, bool is_full_width_, bool allow_scrolling_)
  : url_(std::move(url_))
...
void pageBlockEmbedded::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (poster_photo_ == nullptr) { s.store_field("poster_photo", "null"); } else { poster_photo_->store(s, "poster_photo"); }
    s.store_field("width", width_);
    s.store_field("height", height_);
    if (caption_ == nullptr) { s.store_field("caption", "null"); } else { caption_->store(s, "caption"); }
...
pageBlockEmbeddedPost::pageBlockEmbeddedPost(std::string const &url_, std::string const &author_, object_ptr<photo> &&author_photo_, std::int32_t date_, std::vector<object_ptr<PageBlock>> &&page_blocks_, object_ptr<RichText> &&caption_)
  : url_(std::move(url_))
  , author_(std::move(author_))
...
void pageBlockEmbeddedPost::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (author_photo_ == nullptr) { s.store_field("author_photo", "null"); } else { author_photo_->store(s, "author_photo"); }
    s.store_field("date", date_);
    { const std::vector<object_ptr<PageBlock>> &v = page_blocks_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("page_blocks", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    if (caption_ == nullptr) { s.store_field("caption", "null"); } else { caption_->store(s, "caption"); }
    s.store_class_end();
  }
}
...
pageBlockCollage::pageBlockCollage(std::vector<object_ptr<PageBlock>> &&page_blocks_, object_ptr<RichText> &&caption_)
  : page_blocks_(std::move(page_blocks_))
  , caption_(std::move(caption_))
{}
...
pageBlockSlideshow::pageBlockSlideshow(std::vector<object_ptr<PageBlock>> &&page_blocks_, object_ptr<RichText> &&caption_)
  : page_blocks_(std::move(page_blocks_))
  , caption_(std::move(caption_))
{}
...
passportAuthorizationForm::passportAuthorizationForm(std::int32_t id_, std::vector<object_ptr<passportRequiredElement>> &&required_elements_, std::vector<object_ptr<PassportElement>> &&elements_, std::vector<object_ptr<passportElementError>> &&errors_, std::string const &privacy_policy_url_)
  : id_(id_)
...
void passportAuthorizationForm::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    { const std::vector<object_ptr<passportRequiredElement>> &v = required_elements_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("required_elements", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<object_ptr<PassportElement>> &v = elements_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("elements", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<object_ptr<passportElementError>> &v = errors_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("errors", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
...
void passportRequiredElement::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<passportSuitableElement>> &v = suitable_elements_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("suitable_elements", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
passwordState::passwordState(bool has_password_, std::string const &password_hint_, bool has_recovery_email_address_, bool has_passport_data_, std::string const &unconfirmed_recovery_email_address_pattern_)
  : has_password_(has_password_)
  , password_hint_(std::move(password_hint_))
  , has_recovery_email_address_(has_recovery_email_address_)
  , has_passport_data_(has_passport_data_)
  , unconfirmed_recovery_email_address_pattern_(std::move(unconfirmed_recovery_email_address_pattern_))
{}
...
void passwordState::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("unconfirmed_recovery_email_address_pattern", unconfirmed_recovery_email_address_pattern_);
    s.store_class_end();
  }
}
...
void paymentForm::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (invoice_ == nullptr) { s.store_field("invoice", "null"); } else { invoice_->store(s, "invoice"); }
    s.store_field("url", url_);
    if (payments_provider_ == nullptr) { s.store_field("payments_provider", "null"); } else { payments_provider_->store(s, "payments_provider"); }
    if (saved_order_info_ == nullptr) { s.store_field("saved_order_info", "null"); } else { saved_order_info_->store(s, "saved_order_info"); }
    if (saved_credentials_ == nullptr) { s.store_field("saved_credentials", "null"); } else { saved_credentials_->store(s, "saved_credentials"); }
...
void paymentReceipt::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (invoice_ == nullptr) { s.store_field("invoice", "null"); } else { invoice_->store(s, "invoice"); }
    if (order_info_ == nullptr) { s.store_field("order_info", "null"); } else { order_info_->store(s, "order_info"); }
    if (shipping_option_ == nullptr) { s.store_field("shipping_option", "null"); } else { shipping_option_->store(s, "shipping_option"); }
...
void personalDetails::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (birthdate_ == nullptr) { s.store_field("birthdate", "null"); } else { birthdate_->store(s, "birthdate"); }
...
void photo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<photoSize>> &v = sizes_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("sizes", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void proxies::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<proxy>> &v = proxies_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("proxies", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void replyMarkupShowKeyboard::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::vector<object_ptr<keyboardButton>>> &v = rows_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("rows", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { { const std::vector<object_ptr<keyboardButton>> &v2 = v[i]; const std::uint32_t multiplicity2 = static_cast<std::uint32_t>(v2.size()); const auto vector_name2 = "vector[" + tdlibQt::to_string(multiplicity2)+ "]"; s.store_class_begin("", vector_name2.c_str()); for (std::uint32_t i2 = 0; i2 < multiplicity2; i2++) { if (v2[i2] == nullptr) { s.store_field("", "null"); } else { v2[i2]->store(s, ""); } } s.store_class_end(); } } s.store_class_end(); }
...
void replyMarkupInlineKeyboard::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::vector<object_ptr<inlineKeyboardButton>>> &v = rows_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("rows", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { { const std::vector<object_ptr<inlineKeyboardButton>> &v2 = v[i]; const std::uint32_t multiplicity2 = static_cast<std::uint32_t>(v2.size()); const auto vector_name2 = "vector[" + tdlibQt::to_string(multiplicity2)+ "]"; s.store_class_begin("", vector_name2.c_str()); for (std::uint32_t i2 = 0; i2 < multiplicity2; i2++) { if (v2[i2] == nullptr) { s.store_field("", "null"); } else { v2[i2]->store(s, ""); } } s.store_class_end(); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void richTexts::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<RichText>> &v = texts_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("texts", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
scopeNotificationSettings::scopeNotificationSettings(std::int32_t mute_for_, std::string const &sound_, bool show_preview_)
  : mute_for_(mute_for_)
  , sound_(std::move(sound_))
  , show_preview_(show_preview_)
{}
...
void sessions::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<session>> &v = sessions_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("sessions", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void stickerSet::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("is_viewed", is_viewed_);
    { const std::vector<object_ptr<sticker>> &v = stickers_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("stickers", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<object_ptr<stickerEmojis>> &v = emojis_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("emojis", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void stickerSetInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("is_viewed", is_viewed_);
    s.store_field("size", size_);
    { const std::vector<object_ptr<sticker>> &v = covers_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("covers", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void stickerSets::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("total_count", total_count_);
    { const std::vector<object_ptr<stickerSetInfo>> &v = sets_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("sets", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void storageStatistics::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("size", size_);
    s.store_field("count", count_);
    { const std::vector<object_ptr<storageStatisticsByChat>> &v = by_chat_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("by_chat", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void storageStatisticsByChat::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("size", size_);
    s.store_field("count", count_);
    { const std::vector<object_ptr<storageStatisticsByFileType>> &v = by_file_type_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("by_file_type", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
storageStatisticsFast::storageStatisticsFast(std::int64_t files_size_, std::int32_t file_count_, std::int64_t database_size_)
...
void tMeUrls::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<tMeUrl>> &v = urls_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("urls", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testVectorInt::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int32_t> &v = value_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("value", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testVectorIntObject::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<testInt>> &v = value_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("value", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testVectorString::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::string> &v = value_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("value", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testVectorStringObject::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<testString>> &v = value_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("value", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void updateAuthorizationState::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (authorization_state_ == nullptr) { s.store_field("authorization_state", "null"); } else { authorization_state_->store(s, "authorization_state"); }
    s.store_class_end();
  }
}
...
updateNewMessage::updateNewMessage(object_ptr<message> &&message_, bool disable_notification_, bool contains_mention_)
  : message_(std::move(message_))
...
void updateMessageContent::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("chat_id", chat_id_);
    s.store_field("message_id", message_id_);
    if (new_content_ == nullptr) { s.store_field("new_content", "null"); } else { new_content_->store(s, "new_content"); }
    s.store_class_end();
  }
}
...
void updateScopeNotificationSettings::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (scope_ == nullptr) { s.store_field("scope", "null"); } else { scope_->store(s, "scope"); }
    if (notification_settings_ == nullptr) { s.store_field("notification_settings", "null"); } else { notification_settings_->store(s, "notification_settings"); }
    s.store_class_end();
  }
}
...
void updateDeleteMessages::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("chat_id", chat_id_);
    { const std::vector<std::int64_t> &v = message_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("message_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
...
void updateBasicGroup::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (basic_group_ == nullptr) { s.store_field("basic_group", "null"); } else { basic_group_->store(s, "basic_group"); }
    s.store_class_end();
  }
}
...
void updateSupergroup::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (supergroup_ == nullptr) { s.store_field("supergroup", "null"); } else { supergroup_->store(s, "supergroup"); }
    s.store_class_end();
  }
}
...
void updateSecretChat::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (secret_chat_ == nullptr) { s.store_field("secret_chat", "null"); } else { secret_chat_->store(s, "secret_chat"); }
    s.store_class_end();
  }
}
...
void updateUserFullInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("user_id", user_id_);
    if (user_full_info_ == nullptr) { s.store_field("user_full_info", "null"); } else { user_full_info_->store(s, "user_full_info"); }
    s.store_class_end();
  }
}
...
void updateBasicGroupFullInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("basic_group_id", basic_group_id_);
    if (basic_group_full_info_ == nullptr) { s.store_field("basic_group_full_info", "null"); } else { basic_group_full_info_->store(s, "basic_group_full_info"); }
    s.store_class_end();
  }
}
...
void updateSupergroupFullInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("supergroup_id", supergroup_id_);
    if (supergroup_full_info_ == nullptr) { s.store_field("supergroup_full_info", "null"); } else { supergroup_full_info_->store(s, "supergroup_full_info"); }
    s.store_class_end();
  }
}
...
void updateCall::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (call_ == nullptr) { s.store_field("call", "null"); } else { call_->store(s, "call"); }
    s.store_class_end();
  }
}
...
void updateUserPrivacySettingRules::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (setting_ == nullptr) { s.store_field("setting", "null"); } else { setting_->store(s, "setting"); }
    if (rules_ == nullptr) { s.store_field("rules", "null"); } else { rules_->store(s, "rules"); }
    s.store_class_end();
  }
}
...
void updateInstalledStickerSets::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("is_masks", is_masks_);
    { const std::vector<std::int64_t> &v = sticker_set_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("sticker_set_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void updateTrendingStickerSets::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (sticker_sets_ == nullptr) { s.store_field("sticker_sets", "null"); } else { sticker_sets_->store(s, "sticker_sets"); }
    s.store_class_end();
  }
}
...
void updateRecentStickers::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int32_t> &v = sticker_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("sticker_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void updateSavedAnimations::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int32_t> &v = animation_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("animation_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void updateNewInlineQuery::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("id", id_);
    s.store_field("sender_user_id", sender_user_id_);
    if (user_location_ == nullptr) { s.store_field("user_location", "null"); } else { user_location_->store(s, "user_location"); }
    s.store_field("query", query_);
    s.store_field("offset", offset_);
    s.store_class_end();
  }
...
void updateNewCallbackQuery::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (payload_ == nullptr) { s.store_field("payload", "null"); } else { payload_->store(s, "payload"); }
    s.store_class_end();
  }
}
...
void user::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("phone_number", phone_number_);
    if (status_ == nullptr) { s.store_field("status", "null"); } else { status_->store(s, "status"); }
    if (profile_photo_ == nullptr) { s.store_field("profile_photo", "null"); } else { profile_photo_->store(s, "profile_photo"); }
    if (outgoing_link_ == nullptr) { s.store_field("outgoing_link", "null"); } else { outgoing_link_->store(s, "outgoing_link"); }
    if (incoming_link_ == nullptr) { s.store_field("incoming_link", "null"); } else { incoming_link_->store(s, "incoming_link"); }
    s.store_field("is_verified", is_verified_);
    s.store_field("restriction_reason", restriction_reason_);
...
void userPrivacySettingRules::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<UserPrivacySettingRule>> &v = rules_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("rules", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
userProfilePhotos::userProfilePhotos(std::int32_t total_count_, std::vector<object_ptr<photo>> &&photos_)
  : total_count_(total_count_)
...
void userProfilePhotos::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("total_count", total_count_);
    { const std::vector<object_ptr<photo>> &v = photos_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("photos", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void validatedOrderInfo::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<shippingOption>> &v = shipping_options_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("shipping_options", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void voiceNote::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_bytes_field("waveform", waveform_);
    s.store_field("mime_type", mime_type_);
    if (voice_ == nullptr) { s.store_field("voice", "null"); } else { voice_->store(s, "voice"); }
    s.store_class_end();
  }
}
...
void wallpapers::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<wallpaper>> &v = wallpapers_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("wallpapers", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
webPageInstantView::webPageInstantView(std::vector<object_ptr<PageBlock>> &&page_blocks_, bool is_full_)
  : page_blocks_(std::move(page_blocks_))
  , is_full_(is_full_)
{}
...
void changeImportedContacts::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<contact>> &v = contacts_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("contacts", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void createNewStickerSet::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("name", name_);
    s.store_field("is_masks", is_masks_);
    { const std::vector<object_ptr<inputSticker>> &v = stickers_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("stickers", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void getChatEventLog::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("limit", limit_);
    if (filters_ == nullptr) { s.store_field("filters", "null"); } else { filters_->store(s, "filters"); }
    { const std::vector<std::int32_t> &v = user_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("user_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void getLanguagePackStrings::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("language_pack_id", language_pack_id_);
    { const std::vector<std::string> &v = keys_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("keys", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
getPassportAuthorizationForm::getPassportAuthorizationForm(std::int32_t bot_user_id_, std::string const &scope_, std::string const &public_key_, std::string const &nonce_, std::string const &password_)
  : bot_user_id_(bot_user_id_)
  , scope_(std::move(scope_))
...
void optimizeStorage::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<FileType>> &v = file_types_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("file_types", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    { const std::vector<std::int64_t> &v = chat_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("chat_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    { const std::vector<std::int64_t> &v = exclude_chat_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("exclude_chat_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_field("chat_limit", chat_limit_);
    s.store_class_end();
  }
}
...
void parseTextEntities::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("text", text_);
    if (parse_mode_ == nullptr) { s.store_field("parse_mode", "null"); } else { parse_mode_->store(s, "parse_mode"); }
    s.store_class_end();
  }
}
...
pinSupergroupMessage::pinSupergroupMessage(std::int32_t supergroup_id_, std::int64_t message_id_, bool disable_notification_)
  : supergroup_id_(supergroup_id_)
  , message_id_(message_id_)
  , disable_notification_(disable_notification_)
{}
...
void registerDevice::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (device_token_ == nullptr) { s.store_field("device_token", "null"); } else { device_token_->store(s, "device_token"); }
    { const std::vector<std::int32_t> &v = other_user_ids_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("other_user_ids", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
sendInlineQueryResultMessage::sendInlineQueryResultMessage(std::int64_t chat_id_, std::int64_t reply_to_message_id_, bool disable_notification_, bool from_background_, std::int64_t query_id_, std::string const &result_id_)
  : chat_id_(chat_id_)
  , reply_to_message_id_(reply_to_message_id_)
  , disable_notification_(disable_notification_)
  , from_background_(from_background_)
...
void sendMessageAlbum::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("disable_notification", disable_notification_);
    s.store_field("from_background", from_background_);
    { const std::vector<object_ptr<InputMessageContent>> &v = input_message_contents_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("input_message_contents", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void setCustomLanguagePackString::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("language_pack_id", language_pack_id_);
    if (new_string_ == nullptr) { s.store_field("new_string", "null"); } else { new_string_->store(s, "new_string"); }
    s.store_class_end();
  }
}
...
void setPassportElement::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (element_ == nullptr) { s.store_field("element", "null"); } else { element_->store(s, "element"); }
    s.store_field("password", password_);
    s.store_class_end();
  }
}
...
void setPassportElementErrors::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    s.store_field("user_id", user_id_);
    { const std::vector<object_ptr<inputPassportElementError>> &v = errors_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("errors", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void setTdlibParameters::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    if (parameters_ == nullptr) { s.store_field("parameters", "null"); } else { parameters_->store(s, "parameters"); }
    s.store_class_end();
  }
}
...
void testCallVectorInt::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::int32_t> &v = x_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("x", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testCallVectorIntObject::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<testInt>> &v = x_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("x", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testCallVectorString::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<std::string> &v = x_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("x", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { s.store_field("", v[i]); } s.store_class_end(); }
    s.store_class_end();
  }
}
...
void testCallVectorStringObject::store(TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<testString>> &v = x_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + tdlibQt::to_string(multiplicity)+ "]"; s.store_class_begin("x", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
    s.store_class_end();
  }
}
